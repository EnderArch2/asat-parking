...
@section('topbar-actions')
    ...
    <button id="btnEnterVehicle" onclick="enterVehicle()" class="btn" style="background: linear-gradient(135deg, #d81b60, #8e24aa); color: white; box-shadow: 0 4px 12px rgba(216, 27, 96, 0.3); padding: 10px 20px; font-size: 11px; text-transform: uppercase; border: none; cursor: pointer;">
...
<div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
        ...
        <div style="display: flex; gap: 16px; overflow-x: auto; padding-bottom: 8px; scrollbar-width: none;">
            <div class="clock-widget" style="flex-shrink: 0;">
                <div class="clock-content">
                    <img src="{{ asset('assets/img/parkir.png') }}" alt="main_logo" style="width: 50px; height: 50px; object-fit: contain;">
                    <h3 id="clock-day" style="font-size: 22px; font-weight: 700; margin-bottom: 4px; letter-spacing: 0.5px;" class="text-white"></h3>
            ...
            <div class="location-card" id="location-card-{{ $location->id }}"
                ...
                <div style="width: 50px; height: 50px; margin: 0 auto 16px; background: linear-gradient(135deg, #d81b60, #8e24aa); border-radius: 14px; display: flex; align-items: center; justify-content: center; color: white; font-size: 22px; box-shadow: 0 6px 14px rgba(216, 27, 96, 0.3);">
    ...
    <div class="card" style="height: 100%; display: flex; flex-direction: column;">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; padding: 24px;">
            ...
            <button onclick="viewAllTransactions()" class="btn" style="background: transparent; color: #d81b60; border: 1px solid #d81b60; border-radius: 8px; padding: 8px 16px; font-size: 10px; font-weight: 700; cursor: pointer;">
                VIEW ALL
            </button>
        </div>
...
<script>
    ...
    function selectVehicleType(element, vehicleTypeId, vehicleType) {
        ...
        element.style.background = 'linear-gradient(135deg, #d81b60, #8e24aa)';
    ...
    function exitVehicle() {
        ...
        .then(data => {
            if (data.success) {
                updateAvailability(data.id_lokasi, data.jenis_key, data.remaining);
                removeTicketFromSidebar(data.no_tiket);
                document.getElementById('inputTicketNumber').value = '';
                document.getElementById('inputPoliceNumber').value = '';

                // Get ticket info for modal
                fetch('{{ url("transaction/ticket-info") }}/' + noTiket)
                .then(res => res.json())
                .then(ticketInfo => {
                    Swal.fire({
                        icon: 'success',
                        html: `
                            <div style="text-align:left; font-size:14px; line-height:2.2;">
                                <h2 style="font-size: 20px; font-weight: 800; color: #d81b60; margin-bottom: 16px;">Kendaraan Keluar!</h2>
                                <strong>No Tiket:</strong> ${ticketInfo.no_tiket}<br>
                                <strong>Lokasi:</strong> ${ticketInfo.location}<br>
                                <strong>Jenis:</strong> ${ticketInfo.jenis}<br>
                                <strong>Nomor Polisi:</strong> ${ticketInfo.no_polisi}<br>
                                <strong>Masuk:</strong> ${ticketInfo.masuk}<br>
                                <strong>Keluar:</strong> ${ticketInfo.keluar}<br>
                                <hr style="margin: 12px 0; border: 1px solid #e2e8f0;">
                                <h3 style="font-size: 18px; font-weight: 800; color: #344767; margin: 12px 0;">Total Bayar: Rp ${formatNumber(data.total_bayar)}</h3>
                            </div>
                        `,
                        confirmButtonColor: '#d81b60',
                        confirmButtonText: 'OK'
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div style="text-align:center;">
                                <h2 style="font-size: 24px; font-weight: 800; color: #344767; margin-bottom: 8px;">Kendaraan Keluar!</h2>
                                <h3 style="font-size: 20px; font-weight: 800; color: #d81b60;">Total Bayar: Rp ${formatNumber(data.total_bayar)}</h3>
                            </div>
                        `,
                        confirmButtonColor: '#d81b60',
                        confirmButtonText: 'OK'
    ...
    function removeTicketFromSidebar(noTiket) {
        const item = document.querySelector(`.ticket-item[data-ticket="${noTiket}"]`);
        if (item) {
            item.remove();
        }
