...
class TransactionController extends Controller
{
    ...
    public function exitVehicle(Request $request)
    {
        ...
        // Get actual minutes passed
        $totalMenit = max(1, (int) ceil($masuk->diffInSeconds($now) / 60));

        // Calculate cost using the modified testing function
        ...
        // --- TESTING MODE START ---
        // Save the minutes as hours/days to DB so receipts look correct
        $simulatedTotalJam = $totalMenit;
        $simulatedTotalHari = (int) ceil($simulatedTotalJam / 24);
        // --- TESTING MODE END ---

        $transaction->update([
            'keluar'     => $now,
            'no_polisi'  => $request->no_polisi,
            'total_jam'  => $simulatedTotalJam,
            'total_hari' => $simulatedTotalHari,
    ...
    private function hitungBiaya(int $totalMenit, int $perjamPertama, int $perjamBerikutnya, int $maxPerhari): int
    {
        ...
        // --- TESTING MODE: Trick the function into treating minutes as hours ---
        $totalJam = $totalMenit;

        if ($totalJam <= 24) {
            $total = $perjamPertama + ($perjamBerikutnya * ($totalJam - 1));
        ...
        // If it goes over 24 "hours" (which is 24 minutes in testing mode)
        $totalHari = (int) ceil($totalJam / 24);
        $biayaPerHari = (int) ($maxPerhari * 0.6);
        $biayaTotal = $totalHari * $biayaPerHari;
    ...
    {
        $transaction = Transaction::where('no_tiket', $noTiket)->firstOrFail();
